...
const FIELDS =
	'totalItems,items(id,volumeInfo(title,subtitle,authors,publisher,publishedDate,industryIdentifiers,imageLinks/thumbnail,imageLinks/smallThumbnail,language))';
...
function dropTwins(books: CatalogBook[]) {
	...
	return books.filter((b) => {
		const key = [b.title, b.authors.join(), b.publisher, b.year, b.language]
			.join('|')
			.toLowerCase();
...
export async function searchGoogleBooks(
	...
	const rank = (b: CatalogBook) => (preferLang && b.language === preferLang ? 0 : 1);
	const hits = dropTwins(books)
		.sort((a, b) => rank(a) - rank(b))
		.map((b) => ({ ...b, editions: null }));
	return {
		hits,
